...
redis.call('XADD', KEYS[3], 'MAXLEN', '~', ARGV[6], '*', 'jobId', ARGV[1], 'requestId', ARGV[2])
return {ARGV[1], '1'}
`
...
function parseEvent(id: string, value: Record<string, unknown>): InferenceEvent | null {
  ...
  if (type !== 'meta' && type !== 'token' && type !== 'complete' && type !== 'failed' && type !== 'cancelled') return null
...
export class RedisInferenceJobStore implements InferenceJobStore {
  ...
  async claimNext(consumerName: string): Promise<ClaimedInferenceJob | null> {
    ...
    return job ? { queueEntryId: entry.id, job } : null
  }

  async acknowledge(queueEntryId: string) {
    await this.redis.xack(QUEUE_KEY, CONSUMER_GROUP, queueEntryId)
  }
